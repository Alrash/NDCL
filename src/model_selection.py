...
class IIDAccuracySelectionMethodImbalanced(SelectionMethod):
    name = "imbalanced training-domain validation set"
    ...
    def _step_acc(self, record):
        ...
        for i in itertools.count():
            if f'env{i}_out_acc' not in record:
                break
            if i != test_env:
                val_env_keys.append(i)
        test_in_acc_key = 'env{}_in_acc'.format(test_env)
        is_zeros = 'env{}_out_3zero'.format(val_env_keys[0]) in record
        return {
            'val_acc': np.mean([record[f'env{key}_val_acc'] for key in val_env_keys]),
            'test_acc': record[test_in_acc_key],
            'source_acc': np.mean([record[f'env{key}_out_acc'] for key in val_env_keys]),
            'source_many': np.mean([record[f'env{key}_out_0many'] for key in val_env_keys]),
            'source_median': np.mean([record[f'env{key}_out_1median'] for key in val_env_keys]),
            'source_few': np.mean([record[f'env{key}_out_2few'] for key in val_env_keys]),
            'source_zero': np.mean([record[f'env{key}_out_3zero'] for key in val_env_keys]) if is_zeros else None,
            'test_many': record['env{}_in_0many'.format(test_env)],
            'test_median': record['env{}_in_1median'.format(test_env)],
            'test_few': record['env{}_in_2few'.format(test_env)],
            'test_zero': record['env{}_in_3zero'.format(test_env)] if is_zeros else None,
        }
    ...
    def sweep_acc(self, records):
        ...
        _hparams_accs = self.hparams_accs(records)
        if len(_hparams_accs):
            result = _hparams_accs[0][0]
            # return _hparams_accs[0][0]['test_acc']
            return {k: v for k, v in result.items() if v is not None and k != 'val_acc'}
        else:
            # return {'test_acc': None,
            #         'source_many': None, 'source_median': None, 'source_few': None, 'source_zero': None,
            #         'test_many': None, 'test_median': None, 'test_few': None, 'test_zero': None}
            return {'test_acc': None}
