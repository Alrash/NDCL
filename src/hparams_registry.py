...
def _hparams(algorithm, dataset, random_seed):
    ...
    # RDM tuning
    if algorithm != 'RDM':
        _hparam('resnet_dropout', 0., lambda r: r.choice([0., 0.1, 0.5]))        #default
    else:
        if dataset in ['DomainNet', 'TerraIncognita']:
            _hparam('resnet_dropout', 0., lambda r: r.choice([0., 0.003, 0.03])) #RDM
        else:
            _hparam('resnet_dropout', 0., lambda r: r.choice([0., 0.003]))       #RDM
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        hparams['n_meta_test'] = (int(hparams['n_meta_test'][0]), int(hparams['n_meta_test'][1]))
    ...
    elif algorithm == 'DBIDG':
        _hparam('mixup', 0.2, lambda r: r.uniform(0.1, 0.3))
        _hparam('aug_scaler', 1.5, lambda r: 1.5)
        _hparam('alpha', 1e-1, lambda r: 10**r.uniform(-3, 0))
        _hparam('beta', 1e-1, lambda r: 10**r.uniform(-3, 0))


    elif 'BoDA' in algorithm:
        if dataset == 'DomainNet':
    ...
    if dataset in SMALL_IMAGES:
        ...
        if algorithm == 'RDM':
            _hparam('lr', 5e-5, lambda r: 10**r.uniform(-4.5, -4))  #RDM
        elif algorithm in ['PGrad', 'PGradParallel']:
            _hparam('lr', 5e-5, lambda r: 5e-5)
        else:
            _hparam('lr', 5e-5, lambda r: 10**r.uniform(-5, -3.5))  #original

    if dataset in SMALL_IMAGES:
        _hparam('weight_decay', 0., lambda r: 0.)
    else:
        if algorithm == 'RDM':
            _hparam('weight_decay', 0., lambda r: 10 ** r.uniform(-8, -5))  # RDM, we opted to use low weight decay rates for testing
        elif algorithm in ['PGrad', 'PGradParallel']:
            _hparam('weight_decay', 0., lambda r: 10 ** r.uniform(-6, -4))
        else:
            _hparam('weight_decay', 0., lambda r: 10 ** r.uniform(-6, -2))  # original
    ...
    #     if dataset in ['DomainNet', 'TerraIncognita']:
    #         _hparam('batch_size', 40, lambda r: int(r.uniform(30, 60)))  # for RDM
    #     else:
    #         _hparam('batch_size', 88, lambda r: int(r.uniform(70, 100))) # for RDM
